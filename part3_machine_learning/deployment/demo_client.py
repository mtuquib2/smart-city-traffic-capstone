"""
deployment/demo_client.py - Exercise every API endpoint and save the responses as deployment evidence.

By default the app is called in-process with FastAPI's TestClient (no server needed).
Pass --url http://127.0.0.1:8000 to call a running `python -m deployment.serve` over HTTP instead.

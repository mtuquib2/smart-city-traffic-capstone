"""
recommendation_system/recommender.py - Task 5: Travel-time recommendation system for the I-94 corridor.
...
CLI example
-----------
    python -m recommendation_system.recommender --date 2018-03-13 --earliest 7 --latest 19 --window 2 --weather Snow
"""
...
def write_scenario_report(recommender: TravelRecommender) -> None:
    lines = ["# Recommendation Examples", "",
             "Generated by `recommendation_system/recommender.py` with the champion regressor and proxy-risk classifier.",
    ...
    for title, req in SCENARIOS:
        ...
        lines.append("")
    path = Path(__file__).resolve().parent / "recommendation_examples.md"
    path.write_text("\n".join(lines), encoding="utf-8")
    logger.info("Wrote %d recommendation scenarios to recommendation_system/%s", len(SCENARIOS), path.name)
